    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="storybook-creator-title">
      <section className="relative max-h-[95vh] w-full max-w-xl overflow-y-auto rounded-[2rem] border-4 border-white bg-gradient-to-br from-indigo-50 via-white to-fuchsia-50 p-5 text-slate-900 shadow-2xl sm:p-8">
        ...
        {busy ? (() => {
          ...
          const pageCount = Number(progress?.pageCount || 10);
          ...
          const estimatedTotalSeconds = pageCount === 6 ? 240 : 510;
          ...
          return <div className="pt-5" aria-live="polite" aria-atomic="true">
            ...
            <div className="mt-4 grid gap-2 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-bold leading-relaxed text-amber-900 sm:grid-cols-2"><p><strong>Usually:</strong> {pageCount === 6 ? '3–5' : '7–10'} minutes</p><p><strong>Time left:</strong> about {asMinutes(estimatedRemaining)}</p><p className="sm:col-span-2 text-xs text-amber-800">{pageCount} different pictures and {pageCount + 1} ElevenLabs recordings are made in small batches. Keep this screen open; completed pages are saved as they finish, and a retry continues where it stopped.</p></div>
            <ol className="mt-4 space-y-1 text-sm font-black"><Step stage="planning">Plan the adventure</Step><Step stage="cover">Create the cover picture</Step><Step stage="image">Illustrate {page ? `page ${page} of ${pageCount}` : `all ${pageCount} pages`}</Step><Step stage="narration">Record each page with ElevenLabs</Step><Step stage="complete">Save the book for offline reading</Step></ol>
          </div>;
            ...
            {seriesOptions.length > 0 && <label className="mt-4 block text-sm font-black text-slate-700">Series continuity<select value={seriesId} onChange={(event) => { const next = event.target.value; setSeriesId(next); const selected = seriesOptions.find((item) => item.id === next); if (selected?.visualStyle) setStyle(selected.visualStyle); }} className="mt-2 w-full rounded-xl border-2 border-indigo-100 bg-white px-3 py-3 font-black"><option value="">A brand-new story</option>{seriesOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><span className="mt-1 block text-xs font-bold text-slate-500">{selectedSeries ? selectedSeries.approved ? `Continuing ${selectedSeries.name}: the same approved character and world will be carried into this new topic.` : `Continuing ${selectedSeries.name}: its story details will be reused. Save an approved reference image in Characters &amp; series for exact visual continuity.` : 'Choose a saved character series to keep its look across new topics.'}</span></label>}
            <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800"><BookOpen className="mr-1 inline" size={15} /> {ageBand === '3-4' ? 'Six short pages' : 'Ten pages'}, one illustration and one ElevenLabs narration per page. Generation needs internet.</p>
