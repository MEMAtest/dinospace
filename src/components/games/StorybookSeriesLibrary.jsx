...
import { BookOpen, Check, ImagePlus, Plus, X } from 'lucide-react';
...
const StorybookSeriesLibrary = ({ series = [], onSave, onClose, onUse, onGenerateReference }) => {
  ...
  return (
    <div className="fixed inset-0 z-[61] grid place-items-center bg-slate-950/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="series-library-title">
      <section className="relative max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border-4 border-white bg-gradient-to-br from-amber-50 via-white to-indigo-50 p-5 text-slate-900 shadow-2xl sm:p-8">
        ...
        <div className="flex items-center gap-3"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-amber-100 text-amber-700"><BookOpen size={28} /></div><div><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Story library</p><h2 id="series-library-title" className="text-2xl font-black">Characters &amp; series</h2></div></div>
        {!creating && <>
