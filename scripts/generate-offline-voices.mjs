...
await Promise.all(Array.from({ length: concurrency }, async () => {
  while (cursor < OFFLINE_VOICE_CORPUS.length && !rateLimited) {
    const index = cursor;
    cursor += 1;
    try {
      await generateItem(OFFLINE_VOICE_CORPUS[index], index);
    } catch (error) {
...
console.log(JSON.stringify({ total: OFFLINE_VOICE_CORPUS.length, generated, reused }));
