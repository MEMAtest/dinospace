...
import {
  ...
  hasStorySession, pagesForAgeBand, validateStoryOutline, validateStoryRequest,
} from './_utils.js';
...
const requestStory = async (model, input, apiKey, timeoutMs = 60_000) => {
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: `Write an original, safe picture book for age ${input.ageBand} about this topic: ${input.topic}. Use British English and exactly ${pageCount} pages. Write a concise 25-to-40-word summary. Each page must contain ${range} words. Build a warm beginning, discovery, gentle challenge, cooperative solution and satisfying ending. Visual style: ${STYLE_PROMPTS[input.style]}. ${input.seriesContext ? `This is another story in the approved series ${input.seriesContext.name}. Preserve the character appearance (${input.seriesContext.appearance}), personality (${input.seriesContext.personality}), friends and world (${input.seriesContext.friendsWorld}), and visual style (${STYLE_PROMPTS[input.seriesContext.visualStyle]}).` : ''} Define recurring characters and write a visually distinct, situation-specific prompt for every page, varying action, setting and camera while keeping character identity consistent. Do not include brands, copyrighted characters, frightening peril, violence, adult content, personal data, text or lettering inside images.` },
      ],
      response_format: { type: 'json_schema', json_schema: { name: 'amari_custom_storybook', strict: true, schema: { ...schema, properties: { ...schema.properties, pages: { ...schema.properties.pages, minItems: pageCount, maxItems: pageCount } } } } },
  ...
  try { outline = JSON.parse(content); } catch {
    ...
    try { validateStoryOutline(outline, { ageBand: input.ageBand, expectedPageCount: pagesForAgeBand(input.ageBand) }); } catch (error) {
    error.code = 'OUTLINE_SCHEMA';
    throw error;
  }
